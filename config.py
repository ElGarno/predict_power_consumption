...
logger.info(f"  Token: {'✓ SET' if settings.influxdb_token else '✗ NOT SET'}")
logger.info(f"  Org: {settings.influxdb_org}")
logger.info(f"  Bucket: {settings.influxdb_bucket}")
logger.info(f"")
logger.info(f"Notifications:")
logger.info(f"  Pushover API Token: {'✓ SET' if settings.pushover_api_token else '✗ NOT SET'}")
logger.info(f"  Pushover User Group: {'✓ SET' if settings.pushover_user_group else '✗ NOT SET'}")
...
logger.info(f"")
logger.info(f"AWTRIX Display:")
logger.info(f"  Enabled: {settings.awtrix_enabled}")
logger.info(f"  Host: {settings.awtrix_host}")
logger.info(f"  Port: {settings.awtrix_port}")
logger.info(f"")
...
logger.info(f"  Mode: {settings.prediction_mode}")
if settings.prediction_mode == "daily":
    logger.info(f"  Daily prediction: {settings.daily_prediction_hour}:{settings.daily_prediction_minute:02d}")
else:
    logger.info(f"  Interval: Every {settings.prediction_interval_seconds}s")
logger.info(f"  Model retraining: {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][settings.model_retrain_day]} at {settings.model_retrain_hour}:{settings.model_retrain_minute:02d} (Enabled: {settings.model_retrain_enabled})")
logger.info(f"")
logger.info(f"ML Model:")
logger.info(f"  Path: {settings.model_path}")
logger.info(f"  N Estimators: {settings.model_n_estimators}")
